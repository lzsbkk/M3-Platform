...
from PyQt5.QtGui import QColor, QMouseEvent, QPainter, QPainterPath, QFont
...
class TimelineWidget(QWidget):
    ...
    def __init__(self, eeg_data):
        ...
        self.total_duration = eeg_data.duration
        self.visible_start = 0
        self.visible_duration = min(30, self.total_duration)  # Initial 10 seconds or total duration
        ...
        if self.dragging:
            ...
            for start_time, end_time, _, label in self.events:
                if start_time <= time <= end_time:
                    tooltip_text = f"{label}: {start_time:.3f}s - {end_time:.3f}s"

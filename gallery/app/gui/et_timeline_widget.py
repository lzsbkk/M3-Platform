...
from PyQt5.QtGui import QColor, QMouseEvent, QPainter, QPainterPath, QFont,QPen
...
class TimelineWidget(QWidget):
    ...
    def __init__(self, et_data):
        ...
        self.total_duration = et_data.processed_data['Timestamp'].max()
        self.visible_start = 0
        self.visible_duration = min(5, self.total_duration)  # Initial 5 seconds or total duration
    ...
    def paintEvent(self, e):
        ...
        # Draw events
        for start_time, end_time, color, _ in self.events:
            start_pos = self.time_to_pos(start_time)
            end_pos = self.time_to_pos(end_time)
            painter.setBrush(self.get_qcolor(color))
            
            if start_time == end_time:
                # Draw a thin vertical rectangle for point events
                rect_width = max(2, (self.width() - 22) / self.total_duration / 10)  # Ensure minimum width of 2 pixels
                painter.drawRect(QRectF(start_pos - rect_width/2, 0, rect_width, 22))
            else:
                # Draw a rounded rectangle for interval events
                painter.drawRoundedRect(QRectF(start_pos, 0, end_pos - start_pos, 22), 2, 2)
    ...
    def draw_current_time(self, painter):
        if 0 <= self.current_time <= self.total_duration:
            x = self.time_to_pos(self.current_time)
            
            marker_width = 2
            marker_color = QColor(255, 0, 0, 200)  
            
            time_marker_rect = QRectF(x - marker_width / 2, 0, marker_width, 20)
            
            painter.setPen(Qt.NoPen)
            painter.setBrush(marker_color)
            painter.drawRect(time_marker_rect)
    ...
    def time_to_pos(self, time):
        return 11 + (time / self.total_duration) * (self.width() - 22)

    def pos_to_time(self, pos):
        return max(0, min(((pos - 11) / (self.width() - 22)) * self.total_duration, self.total_duration))
    ...
    def mouseMoveEvent(self, event):
        if self.dragging:
            ...
            new_start = max(0, min(self.visible_start + delta_time, self.total_duration - self.visible_duration))
            ...
            time = self.pos_to_time(event.x())
            for start_time, end_time, _, label in self.events:
                if start_time <= time <= end_time:
                    duration = end_time - start_time
                    tooltip_text = f"{label}: {start_time:.3f}s - {end_time:.3f}s (Duration: {duration:.3f}s)"
    ...
    def wheelEvent(self, event):
        ...
        new_start = max(0, min(new_start, self.total_duration - new_duration))
    ...
    def update_data(self, new_et_data):
        ...
        self.total_duration = new_et_data.processed_data['Timestamp'].max()
        self.visible_duration = min(self.visible_duration, self.total_duration)
        self.visible_start = min(self.visible_start, self.total_duration - self.visible_duration)
        self._adjust_handle_pos()
        self.update()
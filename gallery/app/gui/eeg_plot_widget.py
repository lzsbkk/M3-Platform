...
class EEGPlotWidget(QWidget):
    ...
    def __init__(self, eeg_data, channel, wave_color, parent=None):
    ...
    def draw_axes(self, painter):
        # painter.setPen(QPen(ThemeColor.LIGHT_1.color(), 2))
    ...
    def draw_waveform(self, painter):
        try:
            ...
            extra_time = 0.1  
            start_index = max(0, int((max(self.time_range[0], data_start_time) - data_start_time) * self.eeg_data.sample_rate))
            end_index = min(self.eeg_data.data.shape[1], int((min(self.time_range[1] + extra_time, data_end_time) - data_start_time) * self.eeg_data.sample_rate))
            ...
            target_points = int(self.width() - self.left_margin - self.right_margin)
            data_downsampled, time_downsampled = self.simple_downsample(data, time, target_points)

            self.waveform_drawer.draw(painter, data_downsampled, time_downsampled, self.eeg_data.sample_rate, pixels_per_second, self.wave_color)

            self.draw_out_of_range_mask(painter, data_start_time, data_end_time)

        except Exception as e:
            print(f"Error drawing waveform: {str(e)}")
    ...
    def draw_out_of_range_mask(self, painter, data_start_time, data_end_time):
        clip_rect = QRectF(self.left_margin, self.top_margin, 
                        self.width() - self.left_margin - self.right_margin, 
                        self.height() - self.top_margin - self.bottom_margin)
        painter.setClipRect(clip_rect)
    ...
    def reset_view(self):
        self.time_range = [self.data_start_time, min(self.data_start_time + 10, self.data_end_time)]

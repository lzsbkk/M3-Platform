...
class CoordinateTransformer:
    ...
    def time_to_x(self, time):
        return int(self.widget.left_margin + (time - self.widget.time_range[0]) / 
    ...
    def value_to_y(self, value):
        if np.isnan(value):
            return self.widget.height() - self.widget.bottom_margin
    ...
    def x_to_time(self, x):
        return self.widget.time_range[0] + (x - self.widget.left_margin) / \
               (self.widget.width() - self.widget.left_margin - self.widget.right_margin) * \
               (self.widget.time_range[1] - self.widget.time_range[0])

    def y_to_value(self, y):
        return self.widget.y_range[0] + (self.widget.height() - self.widget.bottom_margin - y) / \
               (self.widget.height() - self.widget.top_margin - self.widget.bottom_margin) * \
               (self.widget.y_range[1] - self.widget.y_range[0])
...
class WaveformDrawer:
    ...
    def draw(self, painter, data, time, sample_rate, pixels_per_second):
